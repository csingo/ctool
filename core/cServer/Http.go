...
func dispatch(router *gin.Engine) {
	...
	parseRoute(routeConf, baseGroup, []string{}, []string{})
}

// parseRoute 解析路由
func parseRoute(conf interface{}, router *gin.RouterGroup, frontMiddlewares, postMiddlewares []string) {
	...
	if routeLen > 0 {
		...
		for i := 0; i < routeLen; i++ {
			...
			parseRoute(route, group, frontMiddlewares, postMiddlewares)
		}
	} else {
		// 路由处理方法参数集合
